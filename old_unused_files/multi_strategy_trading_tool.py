...
class DataFetcher:
    ...
    def get_realtime_price_yahoo(self, symbol: str) -> Optional[float]:
        """Fetch real-time price using Yahoo Finance (free, no rate limits)"""
        ...
        try:
            ...
            try:
                # Method 1: info (most current)
                info = ticker.info
                if 'regularMarketPrice' in info and info['regularMarketPrice']:
                    price = float(info['regularMarketPrice'])
                    logger.info(f"Yahoo Finance real-time price for {symbol}: {price:.5f}")
                    return price
            except:
                pass

            try:
                # Method 2: fast_info (backup)
                fast_info = ticker.fast_info
                if hasattr(fast_info, 'last_price') and fast_info.last_price:
                    price = float(fast_info.last_price)
                    logger.info(f"Yahoo Finance fast price for {symbol}: {price:.5f}")
                    return price
            except:
                pass
            ...
            try:
                ...
                if not hist.empty:
                    ...
                    logger.info(f"Yahoo Finance historical price for {symbol}: {price:.5f}")
                    return price
            except:
                pass

            logger.warning(f"Could not fetch Yahoo Finance price for {symbol}")
            return None

        except Exception as e:
            logger.error(f"Error fetching Yahoo Finance price for {symbol}: {e}")
            return None

    def _convert_to_yahoo_symbol(self, symbol: str) -> str:
        """Convert standard forex symbol to Yahoo Finance format"""

        # Remove slash and add =X suffix for forex
    ...
    def get_current_price(self, symbol: str) -> Optional[float]:
        """Fetch current market price - tries Yahoo Finance first, then TwelveData"""

        # Try Yahoo Finance first (real-time, free, no limits)
        if YFINANCE_AVAILABLE:
            yahoo_price = self.get_realtime_price_yahoo(symbol)
            if yahoo_price is not None:
                return yahoo_price

        # Fallback to TwelveData (rate limited but reliable)
        logger.info(f"Falling back to TwelveData for {symbol}")
        return self._get_twelvedata_price(symbol)

    def _get_twelvedata_price(self, symbol: str) -> Optional[float]:
        """Fetch current market price from TwelveData (original method)"""
        ...
        try:
            ...
            logger.error(f"Error fetching TwelveData price for {symbol}: {e}")
            return None
...
class ZoneDetector:
    ...
    def find_zones(self, symbol: str, h1_df: pd.DataFrame, d1_bias: str, h4_bias: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Scan for zones (only recent data - last 48 hours)
            potential_zones = []

            # Only scan the last 48 H1 bars (48 hours) instead of all historical data
            start_index = max(50, len(h1_df) - 58)  # 48 bars + 10 buffer
            end_index = len(h1_df) - 10

            logger.info(f"Scanning recent zones for {symbol}: bars {start_index} to {end_index} (last {end_index - start_index} hours)")
        ...
        for symbol in self.symbols:
            ...
            try:
                ...
                if zones:
                    ...
                    # Get real-time price using Yahoo Finance (much more accurate)
                    realtime_price = self.data_fetcher.get_current_price(symbol)

                    if realtime_price is None:
                        logger.warning(f"Could not fetch real-time price for {symbol}, using H1 close as fallback")
                        realtime_price = h1_data['close'].iloc[-1]

                    # Check zone freshness first (formed within last 6 hours - more strict)
                    if hasattr(best_zone, 'formed_at') and best_zone.get('formed_at'):
                        ...
                        if time_diff > timedelta(hours=6):
                            logger.info(f"Zone filtered: {symbol} zone too old ({time_diff.total_seconds()/3600:.1f} hours, max: 6.0 hours)")
                            continue

                    # Check if zone entry is within reasonable distance of real-time price
                    entry_price = best_zone["entry"]
                    price_distance = abs(realtime_price - entry_price)
                    max_distance = 0.005  # Reduced to 50 pips for tighter validation

                    if price_distance > max_distance:
                        logger.info(f"Zone filtered: {symbol} entry {entry_price:.5f} too far from real-time {realtime_price:.5f} (distance: {price_distance*10000:.1f} pips, max: {max_distance*10000:.0f} pips)")
                        continue

                    logger.info(f"Zone validated: {symbol} entry {entry_price:.5f} close to real-time {realtime_price:.5f} (distance: {price_distance*10000:.1f} pips, age: {time_diff.total_seconds()/3600:.1f}h)")
